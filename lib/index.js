...
const OVERLAY_TARGET = path.join(os.tmpdir(), 'dsh-cua-overlay.ps1')
...
- If a background action cannot be done at a point (canvas click, WinUI/Chromium with no native edit HWND, unsupported drag), the helper returns background_unavailable: true with an explanatory message; retry that single action with dispatch: "foreground" (real SendInput, brings window forward, focus_ok reports whether focus was confirmed).
- overlay is opt-in (default false, off unless you pass overlay: true): an optional codex-style transparent click-through pill may appear near the target point showing the AI's virtual cursor position. It is disabled by default so it never disturbs your screen.
...
function defineComputerTool(harness) {
  const parameters = {
    ...
    properties: {
      ...
      overlay: { type: 'boolean', description: 'Opt-in: show a codex-style transparent virtual-cursor pill near the target while acting (default false; off unless explicitly true).' },
    },
    required: ['action'],
  }
