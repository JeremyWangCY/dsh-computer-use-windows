...
- overlay is ON by default: before each input action the helper moves a small codex-style click-through mouse-cursor arrow to the target point, so you can see where the AI is about to click/type. It never takes focus and clicks pass through it, and it does not fade — it just moves to the next point. Pass overlay: false on an action to hide the arrow for that action.
