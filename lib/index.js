...
// UIA accessibility tree + PrintWindow screenshots + real SendInput
// mouse/keyboard, executed through a local Windows PowerShell 5.1 helper
// bundled in this package (copied to %TEMP% once per host start).
//
...
const toolDescription = `Desktop computer-use tool for the local Windows session (mirrors zcode/codex computer use). Inspect and operate real desktop apps through the Windows UI Automation accessibility tree, per-window PNG screenshots, and real mouse/keyboard input.

Typical loop:
1. list_apps  -- list running apps (pid, name, windows with title/hwnd/rect).
2. get_app_state { app, screenshot: true }  -- focuses the window, builds an indexed accessibility tree (elements: index/role/name/value/automation_id/rect/invokable), returns document_text when the app exposes it, and saves a window screenshot whose absolute path is in screenshot.path (read it with the read_image tool to see the actual screen). screenshot.window_rect is the window's screen origin.
...
- x/y (click/scroll/drag) are window-local pixels from the top-left of the window; add window.rect.x / window.rect.y to get screen coordinates.
- Input actions bring the target window to the foreground first (focus_ok reports whether focus was confirmed).
