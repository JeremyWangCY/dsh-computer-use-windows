...
- overlay is ON by default: before an input action the helper shows a small codex-style transparent click-through pill near the target point (the AI's virtual cursor), so you can see where it's about to click/type. It never takes focus and clicks pass through it, so it doesn't block or disturb you. Pass overlay: false on an action to hide it for that action.
...
function defineComputerTool(harness) {
  const parameters = {
    ...
    properties: {
      ...
      overlay: { type: 'boolean', description: 'Show the codex-style transparent virtual-cursor pill near the target while acting (default true). Set false to hide it.' },
    },
    required: ['action'],
  }
