...
- overlay defaults to true: before each input action a codex-style transparent click-through pill appears near the target point showing the AI's current virtual cursor position and action label; it fades after ~1s and has an x button that hides it. Set overlay: false to hide it.
...
function defineComputerTool(harness) {
  const parameters = {
    ...
    properties: {
      ...
      overlay: { type: 'boolean', description: 'Show a codex-style transparent virtual-cursor pill near the target while acting (default true).' },
    },
    required: ['action'],
  }
