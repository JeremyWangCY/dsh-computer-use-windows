...
function defineComputerTool(defineTool) {
  ...
  return defineTool({
    ...
    async execute(args, exec) {
      ...
      // ponytail: Windows argv caps at ~32k — chunk long text so a big paste can't fail the spawn
      const field = action === 'type' ? 'text' : action === 'set_value' ? 'value' : null
      const big = field ? args && args[field] : null
      if (typeof big === 'string' && big.length > 6000) {
        const out = { ok: true, action, chunks: 0 }
        for (const part of splitText(big)) {
          out.chunks += 1
          const r = await runAction(action, { ...args, [field]: part }, signal)
